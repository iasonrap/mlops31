...
@pytest.mark.skipif(
    not os.path.exists("data/raw/raw-img/cane/OIF-e2bexWrojgtQnAPPcUfOWQ.jpeg"), reason="Test image not found"
)
def test_classify_endpoint():
    ...
    with open("data/raw/raw-img/cane/OIF-e2bexWrojgtQnAPPcUfOWQ.jpeg", "rb") as image_file:
        response = client.post("/classify/", files={"file": ("test_image.jpg", image_file, "image/jpeg")})
    assert response.status_code == 200
...
def test_post_data_endpoint():
    ...
    mock_data = {"key": "value"}
    response = client.post("/post_data", json=mock_data)
    assert response.status_code == 200
